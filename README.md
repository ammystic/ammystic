### Hello! 👋
My name is **Mystic**

I'm fluent in JavaScript, HTML, CSS, Python & C++

Protocols & Standards I'm familiar with: HTTP(S), WebSockets, JSON Web Tokens, JSON, YAML, Markdown, TOML


Join my [Discord server](https://discord.gg/wK2Y8w7zqW) for updates on things I make

**My PC Specs**:
![Windows 11 Pro](https://img.shields.io/badge/Windows-11_Pro-blue?logo=windows11)
![Ryzen 5 4500X](https://img.shields.io/badge/Ryzen%205-4500X-red?logo=amd)
![GTX 1650](https://img.shields.io/badge/GTX-1650-76b900?logo=nvidia)
![B550M DS3H AC Motherboard](https://img.shields.io/badge/Motherboard-B550M%20DS3H%20AC-orange?logo=PCGamingWiki)
![HyperX DDR4 3200 MHz 32GB RAM](https://img.shields.io/badge/RAM-DDR4_16GB_3200_MHz-red?logoWidth=50&logo=data:image/svg+xml;base64,)

**My Laptop**:
![M2 Macbook Air](https://img.shields.io/badge/M2_MacBook_Air-blue?logo=apple)
<br>
<br>

<!--START_SECTION:waka-->

```txt
From: 11 October 2023 - To: 10 November 2023

JavaScript    11 hrs 12 mins  ################---------   62.05 %
JSON          2 hrs 2 mins    ###----------------------   11.31 %
TypeScript    1 hr 13 mins    ##-----------------------   06.80 %
Python        53 mins         #------------------------   04.96 %
HTML          53 mins         #------------------------   04.93 %
```

<!--END_SECTION:waka-->

All my projects here are for EDUCATIONAL PURPOSES, I am not responsible for any individual(s) using my code for their own purposes. Assume all projects are under [MIT License](https://opensource.org/licenses/MIT) unless otherwise specified.
